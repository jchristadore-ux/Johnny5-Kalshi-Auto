...
# ─────────────────────────────────────────────────────────────────────────────
# v6.0.1: STARTUP CONFIG DRIFT VALIDATION (Step 3)
# Checks env vars against TRADING_DOCTRINE v6.0.0 expected values.
# Logs WARNING only. Does not change behaviour or halt.
# ─────────────────────────────────────────────────────────────────────────────
...
def _validate_config_against_doctrine() -> None:
    ...
    # Doctrine: REQUIRE_AGREE_MOMENTUM must be True
    ...
    # Doctrine: MIN_OB_DEPTH_DOLLARS must be 50.0
    if MIN_OB_DEPTH_DOLLARS != 50.0:
        log.warning(
            "CONFIG DRIFT │ MIN_OB_DEPTH_DOLLARS=%.1f (doctrine: 50.0). "
            "OB depth filter is non-standard.",
            MIN_OB_DEPTH_DOLLARS,
        )
        drift_found = True

    # Doctrine: MINIMUM_CONFIDENCE must be 65
    if MINIMUM_CONFIDENCE != 65:
        log.warning(
            "CONFIG DRIFT │ MINIMUM_CONFIDENCE=%d (doctrine: 65). "
            "Confidence threshold is non-standard.",
            MINIMUM_CONFIDENCE,
        )
        drift_found = True

    # Doctrine: MAX_BET_FRACTION must be 0.10
    ...
    # Doctrine: PROFILE ob_thresh must be 0.70 for QUANT
...
def resolve_open_orders() -> None:
    ...
    try:
        ...
        # v6.0.1: Add cursor_id/created date range to avoid empty result from Kalshi API.
        # The positions endpoint may return nothing without a time anchor.
        ...
        # v6.0.1 Step 1: Debug log — diagnose resolution pipeline
        log.info(
            "RESOLVE DEBUG │ %d settled positions from API, %d open orders tracked, "
            "tickers_open=%s, since=%s",
        ...
        for pos in settled_positions:
            ...
            if matched_oid:
                ...
                # v6.0.1: Log unmatched settled positions for diagnosis
                log.info(
                    "RESOLVE DEBUG │ Settled position ticker=%s has NO matching open_order. "
                    "realized_pnl=%s. Current open tickers: %s",
                    ticker,
                    pos.get("realized_pnl", "?"),
                    [t.get("ticker", "?") for t in open_orders.values()],
                )
...
def main() -> None:
    ...
    # v6.0.1: Validate config against doctrine at startup
